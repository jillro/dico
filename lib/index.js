...
Container.prototype.get = function(name, cb) {
  ...
  // check it is a reference to another service. If it is not, it is
  // in fact a value param which must be returned.
  if ('function' !== typeof this.parameters[name]) {
    if ('@' !== this.parameters[name][0]) {
      return cb(null, this.parameters[name]);
    }
...
Container.prototype.set = function(name, value) {
  var key = (this.ns !== 'global') ? this.ns + '.' + name : name;
